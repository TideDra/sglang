...
async fn route_responses_internal(
    ...
    // Responses API clients continue a conversation with previous_response_id,
    // while the chat pipeline tracks raw tokens with traj_id. Give every new
    // response its own trajectory key and seed it from the previous response so
    // branches remain independently retrievable.
    initialize_response_trajectory(ctx, &request, response_id.as_deref());
...
async fn execute_without_mcp(
    ...
    chat_request.traj_id = response_id.clone();
...
/// Create the trajectory addressed by the new Responses API response ID.
///
/// A continuation clones the previous trajectory instead of moving it so that
/// multiple requests can branch from the same `previous_response_id`.
fn initialize_response_trajectory(
    ctx: &super::context::ResponsesContext,
    ...
    ctx.components
        .trajectory_map
        .insert(response_id.to_string(), trajectory);
}
